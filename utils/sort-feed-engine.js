...
async function sortFeed (fastify,options){
    fastify.get("/feed/", async (request,response) => {
    ...
    // define relevant collections here.
        ...
        console.log("DID, MID", MID,DID)
    // general queries
        let feedDataQuery = {
            user : MID
        }
    // retro Arrays
        let oneWeekRetroArr = retroArr(date.split("-"),5);
        let oneMonthRetroArr = retroArr(oneWeekRetroArr[0].split("-"), 30);
